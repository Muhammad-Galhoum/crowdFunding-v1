...
def showProject(request, id):
    ...
    if request.user: #Show the rate of the user who made login if he already rated this project
...
def home(request):
    ...
    hRatedProjects = []  # High Rated Project
...
def report_comment(request, id):
    if request.method == 'POST': # Check if the user reported this comment before of it's the first time
